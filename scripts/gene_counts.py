...
class gCounter:
     ...
     Module Name:
     gCounter
     Module Description: Counts number of genes in different files.
     '''
        
     def __init__(self,blast_file,out_dir):
        self.blast_file=bam_file
	###TODO : Check the bed file format and throw error if needed"
        self.out_dir=out_dir
	self.span_file=out_dir+"/genes_spanned_with_identity.txt"
	self.cp_file=out_dir+"/gene_copy_numbers.txt"
	# logging
        self.__LOGGER = logging.getLogger('gCounter')
    ...
    def getReadsinRegion(self):
     ...
     def count_genes(self):
        ...
        """
	before = datetime.now()
        data = pd.read_table(self.blast_file, header = None)
        d={}
        for i in data:
            ref_id,ref_start,ref_end,q_id,q_start,q_end,aln,rlen,piden=i.split("\t")
            if ref_id not in d:
                 d[ref_id] = {'num_contigs': 1, 'contigs' : [(q_id, aln) ] }
            else:
...
if __name__ == "__main__":
     ...
     worker = gCounter(args.bam_file,args.bed_file,args.output)
